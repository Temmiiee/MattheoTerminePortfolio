...
export const InteractiveGalaxy: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initStars = () => {
      starsRef.current = [];
      // Réduire le nombre d'étoiles selon la taille de l'écran pour améliorer les performances
      const starCount = Math.min(150, Math.floor((canvas.width * canvas.height) / 15000));
    ...
    // Gérer le mouvement de la souris sur le canvas
    let lastParticleUpdate = 0;
    const PARTICLE_UPDATE_INTERVAL = 50;
    
    const handleCanvasMouseMove = (e: MouseEvent) => {
      const now = performance.now();
      if (now - lastParticleUpdate < PARTICLE_UPDATE_INTERVAL) return;
      ...
      if (Math.random() < 0.15 && particlesRef.current.length < 20) {
    ...
    const animate = (currentTime: number) => {
      ...
      starsRef.current.forEach((star) => {
        ...
        if (isDark) {
          ...
          ctx.fillStyle = `rgba(100, 116, 139, ${twinkleOpacity * 0.8})`;
        }
