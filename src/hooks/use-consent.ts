...
export function useConsent() {
  ...
  useEffect(() => {
    if (shouldLog()) {
      console.info("useConsent: mounting, setting mounted to true");
    }
    setMounted(true);
  }, []);
  ...
  useEffect(() => {
    if (shouldLog()) {
      console.info("useConsent: load preferences effect, mounted =", mounted);
    }
  ...
  const openPreferences = useCallback(() => {
    if (shouldLog()) {
      console.info("useConsent: openPreferences called, setting showBanner to true");
    }
    setShowBanner(true);
